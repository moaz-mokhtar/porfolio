...
use crate::components::svg::{
    flags::{America, Germany, Globe, Japan, Korea},
    themes::{Dark, Light, Rust},
};
...
pub fn nav() -> Html {
    ...
    let cycle_theme = {
        ...
        Callback::from(move |_| match next_theme {
            ...
            "rust" => app_context.theme.dispatch(ThemeAction::Rust),
    ...
    fn handle_nav_icon(app_context: &AppContext) -> &'static str {
        match app_context.theme.current {
			// TODO 2023-08-16: Change icons
            "dark" => "./assets/images/logo_dark.png",
            "light" | _ => "./assets/images/logo.png",
        }
    }
    ...
    <header class="navheader">
        ...
        <span class="logo__text">{ "Moaz bin Mokhtar" }</span>
        </a>
        ...
        <ul class="nav__items">
            ...
            style={ if *show_lang_dropdown { "display:block;" } else { "display:none;" } } >
                ...
                <li onclick={ change_language("jp", app_context.language.clone()) }><Japan />{ "日本語" }</li>
                <li onclick={ change_language("kr", app_context.language.clone()) }><Korea />{ "한국어" }</li>
            </ul>
            </li>
